...
<body>
	...
	<main class="th-layout-main">
		...
		<script>
			...
			/* let vos2 = session.getAttribute("vo2")
			console.log("vos2",vos2)
			function getData() {
				
				for (let i = 0; i < vos2.size() - 1; i += 2) {
						let j = i + 1;
			...
			/* function getData() { // 상세정보보기 눌렀을 때 가져오는 동 아파트 주소 정보
				$.ajax({url : 'http://localhost:8083/GitTest1/getApart.do?name='
									+ addr,
			...
			function getapartallinfo() {
				$
						.ajax({
							url : 'http://localhost:8083/GitTest1/getapartallinfo.do?aptname='
									+ infoapart,
			...
			function searchPlaces() { // 검색데이터 가져오기
				...
				$
						.ajax({
							url : 'http://localhost:8083/GitTest1/getApartSearch.do?name='
									+ searchValue,
