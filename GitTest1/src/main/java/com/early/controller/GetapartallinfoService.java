...
public class GetapartallinfoService implements Command{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)
		...
		for(ApartAllinfoVO temp : list) {
			plainText.append(temp.getApt_name()).append(";").append(temp.getApt_loc()).append(";").append(temp.getApt_realprice()).append(";").append(temp.getApt_size()).append(";").append(temp.getApt_img()).append(";");
		}
		for(ApartAllinfoVO temp : list2) {
			plainText.append(temp.getReview_content()).append("|");
		}
		// System.out.println("test222 : "+plainText);
